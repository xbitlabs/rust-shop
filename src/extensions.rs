...
impl Extensions {
    ...
    ///
    pub fn contains<T: 'static + Sync + Send>(&self) -> bool {
        self.map.contains_key(&TypeId::of::<T>())
    }

    /// Get a reference to an item of a given type.
    pub fn get<T: 'static + Sync + Send>(&self) -> Option<&T> {
        self.map
            .get(&TypeId::of::<T>())
    ...
    pub fn get_mut<T: 'static + Sync + Send>(&mut self) -> Option<&mut T> {
        self.map
            .get_mut(&TypeId::of::<T>())
    ...
    ///
    pub fn remove<T: 'static + Sync + Send>(&mut self) -> Option<T> {
    ...
    pub fn iter(&self) -> std::collections::hash_map::Iter<'_, TypeId, Box<dyn Any + Send + Sync>> {
        self.map.iter()
    }
}
...
fn downcast_owned<T: 'static + Sync + Send>(boxed: Box<dyn Any + Sync + Send>) -> Option<T> {
