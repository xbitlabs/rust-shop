...
lazy_static! {
    static ref WE_CHAT_ACCESS_TOKEN_MANAGER: Mutex<WeChatAccessTokenManager> =
        Mutex::new(WeChatAccessTokenManager::new());
}
lazy_static! {
    ///
    /// 全局配置
...
impl WeChatAccessTokenManager {
    ...
    pub async fn get_access_token(&mut self) -> anyhow::Result<String> {
        ...
        if elapsed_time >= self.token.as_ref().unwrap().expires_in.unwrap() {
            ...
            if self.token.as_ref().unwrap().expires_in.unwrap() - elapsed_time
                < TOKEN_REMAINING_LIFETIME_SECONDS_REFRESH
            {
                Ok(self
                    .token
                    .as_ref()
    ...
    fn get_token_from_response(
        &mut self,
        token_response: WeChatAccessTokenResponse,
    ) -> anyhow::Result<String> {
        return match token_response.errcode {
            Some(_) => Err(anyhow!(token_response.errmsg.unwrap())),
            None => {
...
impl WeChatMiniAppService {
    ...
    pub async fn get_access_token(&self) -> anyhow::Result<String> {
        ...
        match lock_result {
            Ok(mut result) => Ok(result.get_access_token().await?),
            Err(error) => {
    ...
    pub async fn get_user_phone_number(
        &self,
        code: String,
    ) -> anyhow::Result<GetUserPhoneNumberResponse> {
        let https = HttpsConnector::new();
        let client = Client::builder().build::<_, hyper::Body>(https);
        let lock_result = WE_CHAT_ACCESS_TOKEN_MANAGER.lock();
        match lock_result {
            Ok(mut result) => {
                ...
                let req = Request::builder()
                    ...
                    .body(
                        ("{".to_string()
                            + format!("\"code\":\"{}\"", code).to_string().as_str()
                            + "}")
                            .into(),
                    )
                    .unwrap();
