...
impl RequestedPath {
    ...
    pub fn resolve(root_path: impl Into<PathBuf>, request_path: &str,custom_path_resolver: Option<Arc<dyn Fn(&str, &str) -> PathBuf + Send + Sync>>) -> Self {
        ...
        match custom_path_resolver {
            Some(resolver)=>{
                let full_path = resolver(root_path.into().to_str().unwrap(),request_path.to_str().unwrap());
                RequestedPath {
                    full_path,
                    is_dir_request,
                }
